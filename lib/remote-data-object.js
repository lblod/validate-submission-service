...
${sparqlEscapeUri(this.uri)}    a               nfo:RemoteDataObject, nfo:fileDataObject;
    ...
    static async process(triples) {
        const addresses = triples.filter(t => t.predicate.value === NIE('url').value);
        const remoteUris = triples
            .filter(t => t.predicate.value === DCT('hasPart').value)
            .map(t => t.object.value)
            // filter out doubles
            .filter((uri, index, a) => a.indexOf(uri) === index)
            // filter out objects without a nie:url
            .filter(uri => addresses.filter(address => (address.subject.value === uri).length !== 0));
        const remotes = remoteUris
            .map(uri => {
            return new RemoteDataObject({
                uri: uri,
                address: addresses.filter(address => (address.subject.value === uri))[0].object.value
            });
        });
