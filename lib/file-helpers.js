 ...
 */
export async function insertTtlFile(submissionDocument, content) {
  ...
  const logicalUri = cts.PREFIX_TABLE.asj.concat(logicalId);
      ...
      INSERT {
        GRAPH ?g {
          ${sparqlEscapeUri(physicalUri)}
            ...
            dct:creator ${sparqlEscapeUri(cts.SERVICES.validateSubmission)} ;
...
export async function updateTtlFile(
  submissionDocument,
  logicalFileUri,
  content
) {
  ...
  `);
    ...
    console.log(
      `Failed to update TTL resource <${logicalFileUri}> in triplestore.`
    );
    throw e;
  }
}
