    ...
    try {
      ...
      if (uti.isCrossReferencingParentType(documentType)) {
        ...
        for (const crossReference of crossReferenceData) {
          if (isCKB) {
            const predictedChildType = uti.getCrossReferencingChildType(
            ...
            const isCKBRelevant = ab.isCKBRelevantForDecisionType(
              crossReference.documentType,
            );
            const predictedChildType = uti.getCrossReferencingChildType(
              crossReference.documentType,
      ...
      const { logicalFile, physicalFile } = await saveFormTriples(
        this.submittedResource,
        triples,
        this.graph,
      );
