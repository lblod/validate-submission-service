...
function getCrossReferencedDocuments(documentUri, triples) {
  ...
  const artikels = simpleDocumentStore.match(new NamedNode(documentUri), ELI('has_part')).filter((t) => simpleDocumentStore.has(t.subject, RDF('type'), BESLUIT('Artikel'))).map(t => t.object);
