...
import { submitSubmissionForm } from './submission-form';
...
class Submission {
  constructor({ submission, submittedResource, fileUri, ttl }) {
    ...
    this.submittedResource = submittedResource; //Is actually the submissionDocument
  ...
  */
  async submit(form, triples) {
    try {
      ...
      await update(`
      await submitSubmissionForm(this.submittedResource, triples);
          PREFIX meb: <http://rdf.myexperiment.org/ontologies/base/>
          PREFIX prov: <http://www.w3.org/ns/prov#>

          INSERT {
            GRAPH ?g {
              ${sparqlEscapeUri(this.submission)} prov:used ${sparqlEscapeUri(form)} ;
            }
          } WHERE {
            GRAPH ?g {
              ${sparqlEscapeUri(this.submission)} a meb:Submission .
            }
          }
        `);

    } catch (e) {
      console.log(`Something went wrong while submitting submission ${this.submission}`);
      console.log(e);
      throw e;
    }
  }
