...
import { getSourceTtl, getMetaTtl, updateDocument, saveFormTriples } from './submission-form';
import fs from 'fs';
...
class Submission {
  ...
  */
  ...
  async process() {
    try {
      ...
      if (currentStatus) {
        ...
        if (targetStatus)
          await this.postProcess(targetStatus, triples);
  ...
  async postProcess(status, triples) {
    if(status === SENT_STATUS) {
      console.log(`>> Starting Submission post-process <<`);
