...
class Submission {
  ...
  async process() {
    try {
      ...
      const documentType = await getDocumentTypeFromSubmission(this.uri);
      if (uti.isCrossReferencingParentType(documentType)) {
        ...
        const childSubmissionsData = await getCrossReferencedSubmissions(this.uri);
        const crossReferenceData = [];
        for (const { childSubmission, childDocumentType } of childSubmissionsData) {
        ...
        const isCKB = await ab.isCKB(this.organisationId);
        ...
        for (const crossReference of crossReferenceData) {
          if (isCKB) {
            ...
            if (predictedChildType !== crossReference.childDocumentType) {
              isValidCrossReferencing = false;
            }
            const validAuth = await ab.validateCKBEBAuthorisation(crossReference.submission, crossReference.childSubmission);
            if (!validAuth) isValidCrossReferencing = false;
          } else {
            ...
            if (isCKBRelevant) {
              const validAuth = await ab.validateGEBWithRelevantCKBAuthorisation(crossReference.submission, crossReference.childSubmission);
              if (!validAuth) isValidCrossReferencing = false;
            } else {
              const validAuth = await ab.validateGEBAuthorisation(crossReference.submission, crossReference.childSubmission);
              if (!validAuth) isValidCrossReferencing = false;
            }
          }
        }
...
async function getDocumentTypeFromSubmission(submissionUri) {
  const result = await query(`
    PREFIX prov: <http://www.w3.org/ns/prov#>
    ...
    SELECT ?documentType
    WHERE {
      ${sparqlEscapeUri(submissionUri)} prov:generated ?formData .
      ?formData ext:decisionType ?documentType .
    }
    LIMIT 1`);
  return result?.results?.bindings[0]?.documentType?.value;
}

async function getCrossReferencedSubmissions(submissionUri) {
  const result = await query(`
    PREFIX prov: <http://www.w3.org/ns/prov#>
    PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
    PREFIX dcterms: <http://purl.org/dc/terms/>

    SELECT DISTINCT ?childSubmission ?childDocumentType
    WHERE {
      ${sparqlEscapeUri(submissionUri)} prov:generated ?formData .
      ?formData dcterms:relation ?childSubmissionDocument .
      ?childSubmission
        dcterms:subject ?childSubmissionDocument ;
        prov:generated ?childFormData .
      ?childFormData ext:decisionType ?childDocumentType .
    }`);
  if (result?.results?.bindings) {
    return result.results.bindings.map((binding) => {
      return {
        childSubmission: binding.childSubmission.value,
        childDocumentType: binding.childDocumentType.value,
      };
    });
  }
}
