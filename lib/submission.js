...
import { getSourceTtl, getMetaTtl, submitDocument } from './submission-form';
import fs from 'fs';
...
class Submission {
  constructor({ submission, status, submittedResource }) {
    this.uri = submission;
    this.status = status;
  ...
  async process() {
    try {
      ...
      const formBuilder = new FormBuilder({ submittedResource: this.submittedResource, sourceTtl, metaTtl });
      const triples = formBuilder.build().data();

      if (!triples.length) {
        console.log(`No form data could be filled in. Nothing harvested for submission <${this.uri}> with submitted resource <${this.submittedResource}>`);
      }
      ...
      if (currentStatus) {
        if (currentStatus == SUBMITABLE_STATUS) {
          if (!isValid) {
            ...
            await this.submit(formBuilder.form, triples);
   ...
   * - writing form data to a TTL file on disk. Will be used to render the submission form.
  */
  async submit(form, triples) {
    try {
      await submitDocument(this.submissionDocument, triples);
      ...
      await update(`
          ...
          INSERT {
            GRAPH ?g {
              ${sparqlEscapeUri(this.uri)} prov:used ${sparqlEscapeUri(form)} ;
            }
          } WHERE {
            GRAPH ?g {
