...
export default class FormBuilder {
  constructor(submittedResource, formTtl, sourceTtl, metaTtl) {
    ...
    rdflibParse(formTtl, this.store, env.FORM_GRAPH, 'text/turtle');
    rdflibParse(metaTtl, this.store, env.META_GRAPH, 'text/turtle');
    rdflibParse(sourceTtl, this.store, env.SOURCE_GRAPH, 'text/turtle');

    this._submittedResource = submittedResource;
  }

  get formGraph() {
    return new NamedNode(env.FORM_GRAPH);
  }

  get metaGraph() {
    return new NamedNode(env.META_GRAPH);
  }

  get sourceGraph() {
    return new NamedNode(env.SOURCE_GRAPH);
  }
  ...
  data() {
    if (this.hasForm()) return importTriplesForForm(this.form, this.options);
    else return [];
  }
 ...
 */
function bestMatch(forms) {
  if (forms.length) {
    if (forms.length > 1)
      console.log(
        `Found ${forms.length} forms while only 1 was expected. Just taking the first one`
      );
    return forms[0];
  } else {
    return null;
