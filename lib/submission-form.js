...
import * as uti from '../automatic-submission-flow-tools/utils.js';
...
export async function getSourceTtl(submissionDocument) {
  const sourceContent = await getHarvestedData(submissionDocument);
  const additionsContent = await getAdditions(submissionDocument);
  const removalsContent = await getRemovals(submissionDocument);

  const base = await uti.ttlToStore(sourceContent);
  const additions = await uti.ttlToStore(additionsContent);
  const removals = await uti.ttlToStore(removalsContent);

  base.removeQuads([...removals]);
  base.addQuads([...additions]);

  return uti.storeToTtl(base);
}
