...
import * as cro from '../cross-reference-mappings';
...
export function isCrossReferencingParentType(documentType) {
  return !!(
    getCrossReferencingChildType(documentType, false) ||
    getCrossReferencingChildType(documentType, true)
  );
}

export function getCrossReferencingChildType(documentType, isCKB) {
  if (isCKB) return cro.crossReferenceMappingsGemeente_CKB_EB[documentType];
  else return cro.crossReferenceMappingsGemeente_EB[documentType];
}
